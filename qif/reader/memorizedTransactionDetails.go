...
func memorizedTransactionDetails(buf []byte, lineNo int) (totalBytesConsumed int, linesConsumed int, details []*qif.MemorizedTransactionDetail, err error) {
	var detail *qif.MemorizedTransactionDetail
	...
	for len(buf) > 0 && buf[0] != '!' {
		...
		if len(input) == 0 {
			...
			detail = &qif.MemorizedTransactionDetail{}
		}
